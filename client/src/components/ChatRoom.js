...
export default function ChatRoom({ room, currentUser, userStatuses, onRoomUpdated }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!file) return;
    if (file.size > 50 * 1024 * 1024) return alert('파일 크기는 50MB 이하여야 합니다.');
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.messages}>
        ...
        {typing.length > 0 && (
          <div style={styles.typingIndicator}>{typing.join(', ')}님이 입력 중...</div>
        )}
      ...
      <div style={styles.inputArea}>
        ...
        <textarea
          ...
          rows={1}
        />
        <button style={styles.sendBtn} onClick={handleSend} disabled={!input.trim()}>➤</button>
      </div>
...
const styles = {
  ...
  typingIndicator: { fontSize: 12, color: '#666', padding: '4px 8px', fontStyle: 'italic' },
  ...
  inputArea: { display: 'flex', alignItems: 'flex-end', gap: 8, padding: '10px 16px', background: '#fff', borderTop: '1px solid #eee' },
  attachBtn: { background: 'none', border: 'none', cursor: 'pointer', fontSize: 22, padding: '6px', flexShrink: 0 },
